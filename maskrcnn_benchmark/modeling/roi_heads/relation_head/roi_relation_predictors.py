...
from .model_relmix import Cumix_head, get_freq_from_dict
from .utils_relation import layer_init, get_box_info, get_box_pair_info
from maskrcnn_benchmark.data import get_dataset_statistics


@registry.ROI_RELATION_PREDICTOR.register("CumixMotifPredictor")
class CumixMotifPredictor(nn.Module):
    def __init__(self, config, in_channels):
        super(CumixMotifPredictor, self).__init__()
        self.cfg = config
        self.attribute_on = config.MODEL.ATTRIBUTE_ON
        self.num_obj_cls = config.MODEL.ROI_BOX_HEAD.NUM_CLASSES
        self.num_att_cls = config.MODEL.ROI_ATTRIBUTE_HEAD.NUM_ATTRIBUTES
        self.num_rel_cls = config.MODEL.ROI_RELATION_HEAD.NUM_CLASSES
        self.use_label_smoothing = config.MODEL.ROI_RELATION_HEAD.LABEL_SMOOTHING_LOSS
        ...
        self.prd_weights = torch.from_numpy(temp).cuda()
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.use_vision:
            if self.union_single_not_match:
                ...
                prod_rep1 = prod_rep * union_features

        rel_dists = self.rel_compress(prod_rep1)
...
class MotifPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        prod_rep1 = self.post_cat(prod_rep)

        if self.use_vision:
            if self.union_single_not_match:
                prod_rep2 = prod_rep1 * self.up_dim(union_features)
            else:
                prod_rep2 = prod_rep1 * union_features

        rel_dists = self.rel_compress(prod_rep2)
...
class TransformerPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        print(print(union_features.shape))
        print('\n\n\nrel_pair_idxs:#######################################')
        print(rel_pair_idxs)
        print(rel_pair_idxs[0].shape)
        print('\n\n\nrel_labels:#######################################')
        print(rel_labels)
        print(rel_labels[0].shape)
        print('\n\n\nrel_binarys:#######################################')
        print(rel_binarys)
        print(rel_binarys[0].shape)
